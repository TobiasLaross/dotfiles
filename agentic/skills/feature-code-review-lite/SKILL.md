...
description: >-
  ...
  code quality, and completeness against acceptance criteria. Lite flow variant — uses story.md
  acceptance criteria as the requirements source (no impl-plan or test-plan). Outputs structured
  findings to review-fixes.md for follow-up with /feature-code-fix-lite.
...
source instead of impl-plan tasks, and assesses test quality against what was written ad-hoc
rather than against a test-plan.md.
...
There is no `impl-plan.md` or `test-plan.md` in the lite flow.
...
Also read the plan's Design Decisions section for context on intentional choices:
[DESIGN_DECISIONS]
...
Part B — Test Quality:
There is no pre-defined test plan in the lite flow. Tests were written ad-hoc
during implementation. Assess what exists:
...
4. Acceptance criteria coverage — for each acceptance criterion, determine
   whether at least one test validates it: Covered, Missing, or Partial.
...
## Feature Code Review: `<feature-name>` (lite)
...
### Completeness
#### Acceptance Criteria
[Agent 4 Part A — each criterion: Fully / Partially / Not covered,
with explanation]
...
[Agent 4 Part B.4 — X of Y criteria have test coverage, list any
missing/partial]
