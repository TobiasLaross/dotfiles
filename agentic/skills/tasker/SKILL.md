...
# Tasker — Autonomous Task Loop Setup & Launch
...
## Step 6 — Generate TASKER.md

Using the test command from Step 4 (or `UNKNOWN`) and base branch from Step 5,
write `~/.claude/features/<name>/TASKER.md`:

````md
# TASKER — Implementation Loop

You are one iteration of a task loop. You have ONE job: complete the next
...
6. If everything is clean (or only has minor style nits):
   - Append `TASKER_DONE` on its own line to `progress.md`
...
Then say: _"Starting the task loop now. You can Ctrl+C between iterations to
pause — run `tasker <name>` to resume anytime."_

Launch the loop:

```bash
exec "$DOTFILES/scripts/tasker.sh" "<name>"
```

If `$DOTFILES` is not set, use `~/dotfiles/scripts/tasker.sh`.

If the Bash tool times out (long implementations), this is expected — the loop is
resumable. Tell the user: _"Loop timed out in the tool runner. Run
`tasker <name>` from your terminal to continue."_
